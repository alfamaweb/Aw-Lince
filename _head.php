<!-- Meta Data -->
	<!-- Charset  -->
	<meta charset='UTF-8'>

	<!-- Viewport  -->
	<meta name='viewport' content='width=device-width, initial-scale=1.0, user-scalable=no'>

	<!-- Meta  -->
	<title><?php echo $page->title; ?></title>
	<meta name='description' content='<?php echo $page->desc; ?>'>

	<!-- Social: Twitter -->
	<meta name='twitter:card' content='summary_large_image'>
	<meta name='twitter:title' content='<?php echo $page->title ?>'>
	<meta name='twitter:description' content='<?php echo $page->desc; ?>'>
	<meta name='twitter:image:src' content='<?php echo $site->url; ?>/assets/img/thumbnail.jpg'>

	<!-- Social: Facebook / Open Graph -->
	<meta property='og:url' content='<?php echo "http://".$_SERVER["SERVER_NAME"].strtok($_SERVER ["REQUEST_URI"],"?");  ?>'>
	<meta property='og:type' content='website'>
	<meta property='og:title' content='<?php echo $page->title; ?>'>
	<meta property='og:image' content='<?php echo $site->url; ?>/img/social/thumbnail.jpg'/>
	<meta property='og:description' content='<?php echo $page->desc; ?>'>
	<meta property='og:site_name' content='<?php echo $site->name; ?>'>

	<!-- Favicon -->
	<link rel='shortcut icon' type='image/x-icon' href='assets/img/meta/favicon.png'>

	<!-- Icons iOS -->
	<link rel='apple-touch-icon' sizes='57x57' href='assets/img/meta/apple-touch-icon-57x57.png'>
	<link rel='apple-touch-icon' sizes='114x114' href='assets/img/meta/apple-touch-icon-114x114.png'>
	<link rel='apple-touch-icon' sizes='72x72' href='assets/img/meta/apple-touch-icon-72x72.png'>
	<link rel='apple-touch-icon' sizes='144x144' href='assets/img/meta/apple-touch-icon-144x144.png'>
	<link rel='apple-touch-icon' sizes='60x60' href='assets/img/meta/apple-touch-icon-60x60.png'>
	<link rel='apple-touch-icon' sizes='120x120' href='assets/img/meta/apple-touch-icon-120x120.png'>
	<link rel='apple-touch-icon' sizes='76x76' href='assets/img/meta/apple-touch-icon-76x76.png'>
	<link rel='apple-touch-icon' sizes='152x152' href='assets/img/meta/apple-touch-icon-152x152.png'>

	<!-- Icon Windows -->
	<meta name='msapplication-TileColor' content='#ffffff'>
	<meta name='msapplication-TileImage' content='assets/img/meta/mstile-144x144.png'>


<!-- Libs Head -->


<!-- Bootstrap -->
<link rel='stylesheet' type='text/css' href='assets/libs/bootstrap/bootstrap.min.css'>

<!-- Main CSS -->
<link rel='stylesheet' href='assets/css/style.css'>

<!-- Google Analitycs -->


